import React from 'react';
import { Send, Search, FileText, HardHat } from 'lucide-react';

interface ProcessStep {
  number: string;
  title: string;
  description: string;
  icon: React.ElementType;
}

const stepsData: ProcessStep[] = [
  {
    number: '01',
    title: 'Request',
    description: 'Tell us about your project online in 60 seconds or call directly.',
    icon: Send,
  },
  {
    number: '02',
    title: 'Inspect',
    description: 'We conduct a free on-site 17-point roof diagnostic & photo report.',
    icon: Search,
  },
  {
    number: '03',
    title: 'Plan',
    description: 'You receive a clear itemized quote with zero surprise fees & financing options.',
    icon: FileText,
  },
  {
    number: '04',
    title: 'Build',
    description: 'Flawless installation backed by lifetime workmanship warranties.',
    icon: HardHat,
  },
];

export const ProcessSection: React.FC = () => {
  return (
    <section className="w-full bg-white py-16 sm:py-20 lg:py-24 border-t border-slate-100">
      <div className="max-w-[1720px] w-full mx-auto px-4 sm:px-6 lg:px-12 space-y-12">
        
        {/* Section Header */}
        <div className="text-center max-w-2xl mx-auto space-y-3">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-blue-50 border border-blue-100 text-xs font-bold text-[#1D61E7] tracking-wide uppercase">
            <span>OUR SIMPLE PROCESS</span>
          </div>

          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black text-[#0B1220] tracking-tight">
            Transparent 4-Step Process
          </h2>

          <p className="text-slate-600 font-medium text-base sm:text-lg leading-relaxed">
            From initial call to final inspection, we make your roofing project effortless.
          </p>
        </div>

        {/* 4-Step Horizontal Sequence */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 relative">
          
          {/* Connector Line for Desktop */}
          <div className="hidden md:block absolute top-12 left-[12%] right-[12%] h-0.5 bg-slate-100 z-0"></div>

          {stepsData.map((step, idx) => {
            const IconComponent = step.icon;

            return (
              <div
                key={step.number}
                className="bg-[#F8FAFC] rounded-2xl p-6 border border-slate-100 relative z-10 flex flex-col justify-between group hover:border-blue-200 hover:shadow-md transition-all duration-300"
              >
                <div>
                  {/* Step Icon & Number Badge */}
                  <div className="flex items-center justify-between mb-5">
                    <div className="w-12 h-12 rounded-xl bg-white border border-slate-100 text-[#1D61E7] flex items-center justify-center shadow-2xs group-hover:bg-[#1D61E7] group-hover:text-white transition-colors duration-300">
                      <IconComponent className="w-5.5 h-5.5 stroke-[2.2]" />
                    </div>

                    <span className="w-8 h-8 rounded-full bg-blue-50 text-[#1D61E7] font-mono text-xs font-extrabold flex items-center justify-center">
                      {step.number}
                    </span>
                  </div>

                  {/* Title & Description */}
                  <h3 className="text-lg font-bold text-[#0B1220] group-hover:text-[#1D61E7] transition-colors mb-2">
                    {step.title}
                  </h3>

                  <p className="text-xs sm:text-sm text-slate-500 font-medium leading-relaxed">
                    {step.description}
                  </p>
                </div>

                <div className="pt-4 text-xs font-bold text-[#1D61E7] opacity-0 group-hover:opacity-100 transition-opacity">
                  Step {idx + 1} of 4 →
                </div>
              </div>
            );
          })}

        </div>

      </div>
    </section>
  );
};
